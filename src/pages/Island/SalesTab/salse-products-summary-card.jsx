import {
  List,
  TrendingUp,
  TrendingDown,
  DollarSign,
  Fuel,
  ShoppingBag,
  Receipt,
  Info,
} from "lucide-react";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import { Skeleton } from "@/components/ui/skeleton";

export function SummaryCard({
  icon,
  title,
  amount, // For backward compatibility
  primaryAmount, // New prop name
  value, // New prop for simplified API
  secondaryAmount,
  isTotal = false,
  textColor,
  breakdown = [], // Array of {mode: "Cash", amount: "₹2,000"}
  details = [], // New prop for simplified API with [{name, value, quantity}]
  loading = false,
}) {
  // Support different value props
  const displayAmount = value || primaryAmount || amount;

  // Icon color and background combinations
  const getIconStyles = () => {
    switch (icon) {
      case "fuel":
        return {
          bg: "bg-amber-100 border-amber-700 border-[1.3px]",
          text: "text-amber-700",
        };
      case "consumables":
        return {
          bg: "bg-purple-100 border-purple-700 border-[1.3px]",
          text: "text-purple-700",
        };
      case "total":
        return {
          bg: "bg-emerald-100 border-emerald-700 border-[1.3px]",
          text: "text-emerald-700",
        };
      case "income":
        return {
          bg: "bg-emerald-100 border-emerald-700 border-[1.3px]",
          text: "text-emerald-700",
        };
      case "expense":
        return {
          bg: "bg-rose-100 border-rose-700 border-[1.3px]",
          text: "text-rose-700",
        };
      default:
        return {
          bg: "bg-sky-100 border-sky-700 border-[1.3px]",
          text: "text-sky-700",
        };
    }
  };

  const iconStyles = getIconStyles();

  if (loading) {
    return (
      <div className="border rounded-lg p-3 md:p-4 w-full flex-1 h-[100px] md:h-[120px]">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-2 md:gap-3">
            <Skeleton className="h-6 w-6 md:h-7 md:w-7 rounded-md" />
            <div>
              <Skeleton className="h-3 md:h-4 w-16 md:w-20 mb-1 md:mb-2" />
              <Skeleton className="h-5 md:h-6 w-24 md:w-32" />
            </div>
          </div>
          <Skeleton className="h-5 w-5 md:h-6 md:w-6 rounded-md" />
        </div>
      </div>
    );
  }

  // Determine what data to display in the popover
  const popoverData = details.length > 0 
    ? details.map(item => ({ 
        mode: item.name, 
        amount: item.value,
        quantity: item.quantity
      }))
    : breakdown;

  return (
    <div
      className={`border rounded-lg p-3 md:p-4 w-full bg-white flex-1 ${
        isTotal ? "bg-muted/20" : ""
      }`}
    >
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-2 md:gap-3">
          <div
            className={`h-6 w-6 md:h-7 md:w-7 rounded-md flex items-center justify-center ${iconStyles.bg}`}
          >
            {icon === "fuel" && (
              <Fuel className={`h-3.5 w-3.5 md:h-4 md:w-4 ${iconStyles.text}`} />
            )}
            {icon === "consumables" && (
              <ShoppingBag className={`h-3.5 w-3.5 md:h-4 md:w-4 ${iconStyles.text}`} />
            )}
            {(icon === "total" || icon === "income") && (
              <TrendingUp className={`h-3.5 w-3.5 md:h-4 md:w-4 ${iconStyles.text}`} />
            )}
            {icon === "expense" && (
              <TrendingDown className={`h-3.5 w-3.5 md:h-4 md:w-4 ${iconStyles.text}`} />
            )}
          </div>
          <div>
            <p className="text-xs md:text-sm text-muted-foreground">{title}</p>
            <div className={`text-base md:text-xl font-semibold mt-0.5 md:mt-1 ${textColor || ""}`}>
              <span>{displayAmount}</span>
              {secondaryAmount && (
                <>
                  <span className="mx-0.5 md:mx-1">/</span>
                  <span className="text-[14px] md:text-[16px]">{secondaryAmount}</span>
                </>
              )}
            </div>
          </div>
        </div>

        {popoverData.length > 0 && (
          <Popover>
            <PopoverTrigger asChild>
              <button className="p-0.5 md:p-1 rounded-md hover:bg-gray-100 border-gray-300 transition-colors">
                <Info className="h-3.5 w-3.5 md:h-4 md:w-4 text-gray-500" />
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-[180px] md:w-[220px] p-2 md:p-3 shadow-lg">
              <div className="space-y-1 md:space-y-2">
                <h4 className="font-medium text-xs md:text-sm">Breakdown</h4>
                <div className="space-y-0.5 md:space-y-1">
                  {popoverData.map((item, index) => (
                    <div
                      key={index}
                      className="flex justify-between items-center text-[10px] md:text-sm"
                    >
                      <span className="text-muted-foreground">{item.mode}</span>
                      <div className="text-right">
                        <span className={textColor || ""}>{item.amount}</span>
                        {item.quantity && (
                          <div className="text-[8px] md:text-xs text-muted-foreground">
                            {item.quantity}
                          </div>
                        )}
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            </PopoverContent>
          </Popover>
        )}
      </div>
    </div>
  );
}

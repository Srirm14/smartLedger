...
import { Barcode, CalendarDays, IndianRupee, Package2, FileText, Store, Hash } from "lucide-react"
...
import { trackSalesSchema } from "@/lib/schemas/inventory-schema"

export const TrackSalesForm = ({ isOpen, onSave, onClose, inventory, heading, selectedData }) => {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [hasUserMadeChanges, setHasUserMadeChanges] = useState(false)
  
  // Create form with empty initial values
  const form = createZodForm(trackSalesSchema, {
    transaction_id: "",
    quantity: "",
    amount: "",
    date: new Date(),
    ...
    if (isOpen) {
      if (selectedData) {
        ...
        form.reset({
          ...
          salesUnit: selectedData.salesUnit || inventory?.linkedSalesUnits?.[0] || "",
        ...
        form.reset({
          ...
          amount: "",
          date: new Date(),
          salesUnit: inventory?.linkedSalesUnits?.[0] || "",
          reference: "",
          notes: ""
        }, { keepDefaultValues: false })
      }
  ...
  }, [isOpen, selectedData, form, inventory])

  const validateQuantity = (value) => {
    const numValue = Number.parseFloat(value) || 0
    if (numValue > inventory.availableStocks) {
      return `Quantity cannot exceed available stock (${inventory.availableStocks} ${inventory.uom})`
    }
    return true
  }
  ...
  const handleSave = async (data) => {
    ...
    try {
      ...
      if (typeof quantityValidation === "string") {
      ...
      console.error("Failed to track sales:", error)
    } finally {
            ...
            <AccordionContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <SelectField
                  ...
                  options={inventory?.linkedSalesUnits?.map(unit => ({
                    value: unit,
                    label: unit
                  })) || []}
